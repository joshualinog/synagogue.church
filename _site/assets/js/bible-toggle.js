...
(function(){
  ...
  function buildButtons(slug) {
    ...
    nirv.className = 'nirv-btn px-3 py-1 rounded bg-white text-gray-900 border mr-2';
  ...
  function renderVerses(block, verses) {
    if (!verses || !verses.length) {
      block.innerHTML = '<div class="text-sm text-gray-600">No passage available.</div>';
      return;
    }
    block.innerHTML = verses.map(v => `<div class="verse"><span class="verse-num font-bold">${v.chapter}:${v.verse}</span> <span class="verse-text">${v.text}</span></div>`).join('');
  }
  ...
  async function loadAndApplyVersion(slug, version) {
    try {
      ...
      if (!resp.ok) return console.warn('Failed to fetch per-reading JSON', url);
      const data = await resp.json();
      const torahBlock = document.querySelector('.bible-block[data-section="torah"]');
      const gospelBlock = document.querySelector('.bible-block[data-section="gospel"]');
      if (torahBlock && data.torah && data.torah.versions && data.torah.versions[version]) {
        renderVerses(torahBlock, data.torah.versions[version]);
      }
      if (gospelBlock && data.gospel && data.gospel.versions && data.gospel.versions[version]) {
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    buttons.addEventListener('click', function(e){
      ...
      if (ver === 'leb') {
        ...
        if (torahBlock && original.torah !== null) torahBlock.innerHTML = original.torah;
        if (gospelBlock && original.gospel !== null) gospelBlock.innerHTML = original.gospel;
        return;
      }
