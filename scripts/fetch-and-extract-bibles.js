...
if (!fs.existsSync(CONFIG)) {
  ...
  console.log(
    JSON.stringify(
      { leb: "https://example.org/bibledata-leb.zip", nirv: "path-or-url" },
      null,
      2
    )
  );
  console.log(
    "\nOr place zip files in",
    ZIPS_DIR,
    "and reference them by relative path in the config."
  );
  process.exit(0);
}
...
function parseVersesTable(fp) {
  ...
  const textIdx =
    hdr.indexOf("text") >= 0 ? hdr.indexOf("text") : hdr.indexOf("verse_text");
  if (bookIdx === -1 || chapIdx === -1 || verseIdx === -1 || textIdx === -1)
    return {};
  const out = {};
...
(async function main() {
  for (const [key, src] of entries) {
    try {
      ...
      console.log(
        "No usable JSON/TSV found in extracted archive for",
        key,
        ". You may need to convert source to JSON or ask me to add a parser for the source format."
      );
    } catch (e) {
