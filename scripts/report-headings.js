...
const DATA_DIR = path.join(__dirname, '..', 'src', '_data', 'bible', 'per-reading')
...
function scan() {
  ...
  console.log('Found', report.length, 'heading-marked verse entries. Report written to', outFile)
  // print first 10 examples
  report.slice(0, 10).forEach((r, i) => {
    console.log(`${i + 1}. ${r.slug} ${r.section} ${r.translation} ${r.chapter}:${r.verse} -> ${r.text}`)
  })
}
