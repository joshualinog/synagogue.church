...
function canonicalLookup(biblePath, passageStr) {
  ...
  return { bibleKey: path.basename(biblePath), bookKey, chapter: ch, verse: v, text };
}
    ...
    for (const sectionKey of Object.keys(j)) {
      ...
      for (const verName of Object.keys(versions)) {
        ...
        for (const entry of arr) {
          if (!entry) continue;
          if (String(entry.chapter) === String(spec.startChapter) && String(entry.verse) === String(spec.startVerse)) {
            hits.push({ file: f, section: sectionKey, version: verName, text: entry.text, isHeading: !!entry.isHeading });
          }
        }
      }
    }
...
function runOne(passage) {
  ...
  console.log('\nCanonical:');
  if (leb) console.log('LEB:', leb.text || '(empty)'); else console.log('LEB: not found');
  if (nirv) console.log('NIRV:', nirv.text || '(empty)'); else console.log('NIRV: not found');

  const specs = parsePassage(passage);
  if (!specs.length) return;
  const spec = specs[0];
  const perDir = path.join(__dirname, '..', 'src', '_data', 'bible', 'per-reading');
  ...
  for (const h of hits.slice(0, 50)) {
    console.log(`- ${h.file} [${h.section}] ${h.version} -> isHeading=${h.isHeading}`);
